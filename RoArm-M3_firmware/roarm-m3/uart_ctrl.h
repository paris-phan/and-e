void jsonCmdReceiveHandler() {
	...
	switch(cmdType) {
	  ...
	  case CMD_XYZT_GOAL_CTRL: 
		// Note: Removed the roll argument so that only (x, y, z, t, g, spd) are passed.
	  ...
	  case CMD_EOAT_GRAB_TORQUE:
		RoArmM3_handTorqueCtrl(jsonCmdReceive["tor"]);
		break;
	  ...
	  case CMD_DELAY_MILLIS:
		RoArmM3_delayMillis(jsonCmdReceive["cmd"]);
		break;
		
	  case CMD_DYNAMIC_ADAPTATION:
		// Removed the commented-out roll value. Now passes 6 arguments.
	  ...
	  case CMD_MISSION_CONTENT:
		missionContent(jsonCmdReceive["name"]);
		break;
	  ...
	  case CMD_ESP_NOW_ADD_FOLLOWER:
		registerNewFollowerToPeer(jsonCmdReceive["mac"]);
		break;
	  case CMD_ESP_NOW_REMOVE_FOLLOWER:
		deleteFollower(jsonCmdReceive["mac"]);
		break;
