...
void configEEmodeType(byte inputMode) {
  ...
  if (inputMode == 0) {
    ...
    // EoAT_A and EoAT_B remain unchanged.
  ...
  lastZ = goalZ;
  lastT = goalT;
  // Updated: remove the roll argument (initR) from the coordinate control call.
  RoArmM3_baseCoordinateCtrl(initX, initY, initZ, initT, initG);
  RoArmM3_goalPosMove();
}
