...
public class GoalManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Make sure hand tracking is disabled at start
        XRHandToRoArmController[] handControllers = FindObjectsOfType<XRHandToRoArmController>(true);
        if (handControllers != null && handControllers.Length > 0)
        {
            foreach (XRHandToRoArmController controller in handControllers)
            {
                controller.gameObject.SetActive(false);
                Debug.Log($"Disabled {controller.handType} hand tracking during onboarding");
            }
        }
    }
    ...
    public void ForceEndAllGoals()
    {
        ...
        // TODO trigger hand tracking to start here
        // Find and enable XRHandToRoArmController scripts on both hand anchors
        XRHandToRoArmController[] handControllers = FindObjectsOfType<XRHandToRoArmController>(true);
        if (handControllers != null && handControllers.Length > 0)
        {
            foreach (XRHandToRoArmController controller in handControllers)
            {
                ...
                Debug.Log($"Enabled {controller.handType} hand tracking");
            }
        }
        else
        {
            Debug.LogWarning("No XRHandToRoArmController components found in the scene");
        }
    ...
    public void ResetCoaching()
    {
        ...
        // Disable hand tracking when coaching is reset
        XRHandToRoArmController[] handControllersReset = FindObjectsOfType<XRHandToRoArmController>(true);
        if (handControllersReset != null && handControllersReset.Length > 0)
        {
            foreach (XRHandToRoArmController controller in handControllersReset)
            {
                controller.gameObject.SetActive(false);
                Debug.Log($"Disabled {controller.handType} hand tracking on coaching reset");
            }
        }
